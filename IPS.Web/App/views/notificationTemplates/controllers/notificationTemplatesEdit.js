(function () {
    ...
    angular
        ...
        .config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
            $stateProvider
                .state('home.notificationTemplates.edit', {
                    url: "/edit/:notificationId",
                    templateUrl: "views/notificationTemplates/views/notificationTemplatesEdit.html",
                    controller: "notificationTemplatesEditCtrl",
                    resolve: {
                        notification: function (notificationManager, $stateParams) {
                            return notificationManager.getNotificationById($stateParams.notificationId).then(function (data) {
                                return data;
                            });
                        },
        ...
        .controller('notificationTemplatesEditCtrl', notificationTemplatesEditCtrl);

    notificationTemplatesEditCtrl.$inject = ['$scope', 'cssInjector', '$location', 'notificationManager', 'apiService', 'notification', 'cultures', 'evaluationRoles', 'stageTypes', 'templateTypes', 'organizations', 'templateTypeEnum', 'stateTypes', 'stageTypesEnum', '$translate'];

    function notificationTemplatesEditCtrl($scope, cssInjector, $location, notificationManager, apiService, notification, cultures, evaluationRoles, stageTypes, templateTypes, organizations, templateTypeEnum, stateTypes, stageTypesEnum, $translate) {
        ...
        $scope.applyTheme = function () {
            ...
            if ($scope.notification.emailBody.indexOf('<table bgcolor="#425464"') == -1) {
                $scope.notification.emailBody = themeContent();
                $scope.isApplyTheme = false;
            }
        }
        $scope.changeCulture = function () {
            if ($scope.notification && $scope.notification.culture && $scope.cultures) {
                $scope.notification.culture = getById($scope.notification.cultureId, $scope.cultures);
            }
        }
        ...
        function save() {
            isEdit() ? updateNotification() : '';
        }

        function remove() {
            if ($scope.notification) {
                notificationManager.removeNotification($scope.notification.id).then(
                    function (data) {
        ...
        function updateNotification() {
            ...
            notificationManager.updateNotification($scope.notification).then(
                function (data) {
                    notificationManager.returnToPerviousPage();
                },
                function (data) {
        ...
        function setCulture() {
            if ($scope.notification && $scope.notification.culture && $scope.cultures) {
                $scope.notification.culture = getById($scope.notification.culture.id, $scope.cultures);
            }
        }
        ...
        $scope.isApplyTheme = $scope.notification.emailBody.indexOf('<table bgcolor="#425464"') == -1 ? true : false;
        ...
        function themeContent() {
            ...
            if ($scope.notification.culture.cultureName == "nb-NO") {
