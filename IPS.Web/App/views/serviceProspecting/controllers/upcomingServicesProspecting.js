angular.module('ips.serviceProspecting')
    .config(['$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
        var upcomingServiceProspectingResolve = {
        ...
        $stateProvider
            .state('home.serviceProspecting.serviceProspecting', {
                url: "/upcomingServiceProspecting",
                templateUrl: "views/serviceProspecting/views/upcomingServiceProspecting.html",
                controller: "UpcomingServiceProspectingCtrl",
                resolve: upcomingServiceProspectingResolve,
                data: {
    ...
    .controller("UpcomingServiceProspectingCtrl", ['$scope', 'cssInjector', '$stateParams', '$location', 'todoManager', 'todosManager', 'serviceProspectingManager', 'dialogService', 'progressBar', 'localStorageService', '$compile', 'projectRolesEnum', '$translate', 'prospectingTypesEnum', 'globalVariables',
        function ($scope, cssInjector, $stateParams, $location, todoManager, todosManager, serviceProspectingManager, dialogService, progressBar, localStorageService, $compile, projectRolesEnum, $translate, prospectingTypesEnum, globalVariables) {
                            ...
                            _.each(data, function (item) {
                                if (item.taskId) {
                                    ...
                                    serviceProspectingManager.getSkillsByProspectingGoalId(item.id).then(function (data) {
                                        ...
                                        if (moment(kendo.parseDate(item.goalStartDate)).isAfter(moment($scope.endOfDay))) {
            ...
            $scope.CalculateGoalByGoalId = function (prospectingGoalId) {
                ...
                var todayActivities = _.filter(activities, function (activityItem) {
                    return kendo.parseDate(activityItem.activityStart) >= $scope.todayStartDate && kendo.parseDate(activityItem.activityEnd) <= $scope.endOfDay
                });
            ...
            $scope.showFilteredProspectingGoalActivities = function (item) {
                ...
                if ($scope.activityFilterValue) {
                    if ($scope.activityFilterValue == $scope.ActivityFilterOptionsEnum.UpComing) {
